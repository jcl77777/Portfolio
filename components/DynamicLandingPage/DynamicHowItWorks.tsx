...
export const DynamicHowItWorks: React.FC<Props> = ({ data, productId }) => {
  ...
  const lineGradientFrom = isSparks ? 'from-sparks-yellow' : 'from-purple-500'
  const lineGradientTo = isSparks ? 'to-sparks-orange' : 'to-purple-600'
  const lineGradientFromDark = isSparks ? 'dark:from-sparks-yellow' : 'dark:from-purple-400'
  const lineGradientToDark = isSparks ? 'dark:to-sparks-orange' : 'dark:to-purple-500'
  const shadowColor = isSparks ? 'dark:shadow-sparks-orange/50' : 'dark:shadow-purple-500/50'
          ...
          {data.steps.map((step, index) => (
            <div key={index} className="relative">
              ...
              {index < data.steps.length - 1 && (
                <div
                  className={`hidden md:block absolute top-1/2 left-full h-1 bg-gradient-to-r ${lineGradientFrom} ${lineGradientTo} ${lineGradientFromDark} ${lineGradientToDark} -translate-y-1/2 dark:shadow-lg ${shadowColor}`}
              ...
              <div className="relative bg-gray-50 dark:bg-gray-800 rounded-2xl p-8 text-center hover:shadow-xl transition-shadow">
                ...
                <div className={`absolute -top-4 -right-4 w-12 h-12 ${gradientClass} rounded-full flex items-center justify-center text-white font-bold text-xl font-rounded shadow-lg`}>
                  {step.number}
                </div>
