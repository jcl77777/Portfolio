          ...
          {data.steps.map((step, index) => (
            <div key={index} className="relative">
              ...
              {index < data.steps.length - 1 && (
                <div
                  className={`hidden md:block absolute top-1/2 left-full h-1 bg-gradient-to-r ${theme.lineGradientFrom} ${theme.lineGradientTo} ${theme.lineGradientFromDark} ${theme.lineGradientToDark} -translate-y-1/2 dark:shadow-lg ${theme.shadowColor}`}
              ...
              <div className="relative bg-gray-50 dark:bg-gray-800 rounded-2xl p-8 text-center hover:shadow-xl transition-shadow">
                ...
                <div className={`absolute -top-4 -right-4 w-12 h-12 ${theme.gradient} rounded-full flex items-center justify-center text-white font-bold text-xl font-rounded shadow-lg`}>
                  {step.number}
                </div>
